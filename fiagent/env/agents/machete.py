...
class Machete(object):
    ...
    def processing(self):
        ...
        if (opp.getEnergy() >= 300) and (my.getHp() - opp.getHp() <= 300):
        ...
        elif not my_state.equals(
            self.gateway.jvm.enumerate.State.AIR
        ) and not my_state.equals(self.gateway.jvm.enumerate.State.DOWN):
            # If not in air
            if distance > 150:
            ...
            (distance <= 150)
            and (
                my_state.equals(self.gateway.jvm.enumerate.State.AIR)
                or my_state.equals(self.gateway.jvm.enumerate.State.DOWN)
            )
